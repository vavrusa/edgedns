...
impl From<&Question<ParsedDname>> for CacheKey {
    fn from(q: &Question<ParsedDname>) -> Self {
        Self {
            qname: q.qname().clone().into_bytes(),
            qtype: q.qtype(),
            qclass: q.qclass(),
        }
    }
...
impl fmt::Display for CacheKey {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            ...
            self.qclass,
            self.qtype,
            Dname::from_slice(&self.qname).unwrap()
        )
    }
}
...
impl CacheEntry {
    ...
    pub fn ttl(&self) -> u32 {
        let expiration = self.inception.add(Duration::from_secs(u64::from(self.ttl)));
        let now = Instant::recent();
        if now >= expiration {
            0
        } else {
            expiration.sub(now).as_secs() as u32
        }
    }
...
impl Cache {
    ...
    pub fn insert(&mut self, cache_key: CacheKey, entry: CacheEntry) -> bool {
...
mod test {
    ...
    fn insert_normal_1k(b: &mut Bencher) {
        let keys : Vec<_> = DOMAINS.iter().map(|dname| CacheKey::from((dname, Rtype::A, Class::In))).collect();
