...
extern fn debug(ptr: u32, len: u32, ctx: &mut Ctx) {
    let memory = ctx.memory(0);
    let slice = &memory[ptr as usize..(ptr + len) as usize];
    match std::str::from_utf8(slice) {
        Ok(s) => info!("[]: {}", s),
        Err(e) => info!("[]: {:?} (error: {})", slice, e),
    }
}

extern fn register_future(data: i32, vtable_ptr: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();
    let task_id = state.create_task();
  match state.get_tasks_mut().get_mut(task_id as usize) {
    Some(ref mut fut) => {
      trace!("[] registered #{} callback: {}/{}", task_id, data, vtable_ptr);
    ...
    None => {
      warn!("[] attempted to register invalid #{}", task_id);
      guest::Error::NotFound.into()
    }
  }
}

extern fn register_on_message(data: i32, vtable_ptr: i32, ctx: &mut Ctx) {
    let state: &mut SharedState = ctx.into();
...
extern fn timer_poll(task_id: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();
  match state.get_tasks().get(task_id as usize) {
    Some(fut) => {
    ...
    None => {
      warn!("[] polled #{} which does not exist", task_id);
      return guest::Async::Error(guest::Error::NotFound).into();
    }
  }

  // Complete the task
  drop(state.remove_task(task_id as usize));
  guest::Async::Ready(0.into()).into()
}

extern fn timer_create(ms: i32, ctx: &mut Ctx) -> i32 {
  ...
  tokio::spawn(Delay::new(when).then(move |res| {
    match res {
      Ok(_) => {
        trace!("[] timer fired for #{}", task_id);
        if let Some(ref mut task) = state.get_tasks_mut().get_mut(task_id) {
          task.data = Some(Vec::new());
        }
      }
      Err(e) => {
        error!("[] timer error for #{}, error: {:?}", task_id, e);
        drop(state.remove_task(task_id));
      }
    };
    task.notify();
    Ok(())
...
extern fn forward_create(
  ...
  let (msg, scope) = if msg_len == 0 {
    ...
    if let Some((ref scope, ..)) = state.get_requests().get(request as usize) {
  ...
  let task_id = state.create_task();
  trace!("[] forwarding msg to {:?} ({} bytes)", upstream, msg.len());
    ...
    forwarder::Builder::new()
      ...
      .then(move |res| {
        match res {
          ...
          Ok(msg) => {
            trace!("[] response for #{} ({} bytes)", task_id, msg.len());
            if let Some(ref mut task) = state.get_tasks_mut().get_mut(task_id) {
          ...
          Err(e) => {
            error!("[] when forwarding #{}, error: {:?}", task_id, e);
            drop(state.remove_task(task_id));
          }
        };
...
extern fn forward_poll(task_id: i32, msg_ptr: i32, msg_max_len: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();

  // Check if the upstream provided a message
  let msg_len = match state.get_tasks().get(task_id as usize) {
    Some(fut) => {
      match fut.data {
  ...
  let completed_task = state.remove_task(task_id as usize);
  ...
  // Copy response message to guest memory
    ...
    trace!("[] forward done ({} bytes)", msg_len);
...
extern fn request_query_name(request: i32, ptr: i32, max_len: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();
  if let Some((ref scope, ..)) = state.get_requests().get(request as usize) {
...
extern fn request_query_type(request: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();
  if let Some((ref scope, ..)) = state.get_requests().get(request as usize) {
...
extern fn request_set_response(request: i32, ptr: i32, len: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();
    let memory = ctx.memory(0);
    let slice = &memory[ptr as usize .. (ptr+len) as usize];

  if let Some((_, ref mut answer)) = state.get_requests_mut().get_mut(request as usize) {
    trace!("[] setting response for {} ({} bytes)", request, slice.len());
    answer.clear();
...
extern fn local_socket_open(ptr: i32, len: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();
    let memory = ctx.memory(0);
    let slice = &memory[ptr as usize .. (ptr+len) as usize];
  ...
  if !path.exists() {
    trace!("[] local stream {:?}, doesn't exist", path);
    return guest::Error::NotFound.into();
  }
  ...
  trace!("[] connecting local stream {:?}", path);
  let src = GuestStream{host_state: state.clone(), task_handle: task_id};
  ...
  tokio::spawn(UnixStream::connect(&path)
    ...
    .then(move |res| {
      // Log stream closure
      match res {
        Ok(_) => trace!("[] local stream {:?}, closed", task_id),
        Err(e) => warn!("[] error in stream #{}: {:?}, closed", task_id, e),
      };
      Ok(())
    })
  );
...
extern fn local_socket_send(task_id: i32, ptr: i32, len: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();
  ...
  if len == 0 {
    if let Some(task) = state.get_tasks_mut().get_mut(task_id as usize) {
      ...
      if let Some(waiting) = task.waiting.take() {
        trace!("[] local stream #{}, flushed", task_id);
        waiting.notify();
      }
...
extern fn local_socket_recv(_task_id: i32, _ptr: i32, _len: i32, _ctx: &mut Ctx) -> i32 {
  unimplemented!()
}

extern fn local_socket_close(task_id: i32, ctx: &mut Ctx) -> i32 {
    let state: &mut SharedState = ctx.into();

  // Set the closed flag to initiate teardown
  trace!("[] local stream #{}, closing", task_id);
  if let Some(task) = state.get_tasks_mut().get_mut(task_id as usize) {
    task.closed = true;
    if let Some(waiting) = task.waiting.take() {
      waiting.notify();
